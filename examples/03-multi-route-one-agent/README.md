...
client ─▶ edge (:443) ─┬─ app.example.com ─mTLS─▶ agent ─▶ 127.0.0.1:8080
                       └─ api.example.com ─mTLS─▶       └─▶ 127.0.0.1:9090
```
...
- Both edge routes carry the same `agent_fingerprint`; the derived allowlist has
  a single agent.
...
```
curl https://app.example.com/    # -> 127.0.0.1:8080
curl https://api.example.com/     # -> 127.0.0.1:9090
```
